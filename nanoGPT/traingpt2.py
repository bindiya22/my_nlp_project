...
# Hyperparameters
batch_size = 4  # Reduced batch size (to prevent overfitting)
block_size = 512
initial_lr = 1e-3  # Start with 1e-3
mid_lr = 3e-4  # Reduce to 3e-4 after 300 iterations
final_lr = 1e-4  # Reduce to 1e-4 after 750 iterations
max_iters = 2000
save_interval = 100
val_interval = 100  # Print validation loss every 50 iterations
...
# Load GPT-2 model with dropout to prevent overfitting
model_name = "gpt2"
model = GPT2LMHeadModel.from_pretrained(model_name, hidden_dropout_prob=0.1, attention_probs_dropout_prob=0.1)
tokenizer = GPT2Tokenizer.from_pretrained(model_name)
model.to(device)

# Optimizer and Scheduler
optimizer = AdamW(model.parameters(), lr=initial_lr)
scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=100, num_training_steps=max_iters)
...
for iter in range(max_iters):
    ...
    if iter == 350:
        for param_group in optimizer.param_groups:
            param_group['lr'] = mid_lr  # Reduce to 3e-4
    elif iter == 750:
        for param_group in optimizer.param_groups:
